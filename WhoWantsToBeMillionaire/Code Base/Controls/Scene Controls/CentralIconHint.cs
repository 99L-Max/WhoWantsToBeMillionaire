...
namespace WhoWantsToBeMillionaire
{
    class CentralIconHint : BoxAnimationRotation
    {
        ...
        public async Task ShowIcon(HintTypes type, bool playSound)
        {
            var icon = SpritePainter.GetSprite(Resources.Hint_Icons, 3, 6, 0, (int)type);
        ...
        private async Task ShowAnimation(Image icon, bool isShow, bool playSound)
        {
            ...
            var data = JsonReader.GetObject<AnimationRotationData[]>(Resources.AnimationData_CentralIcon);

            using (var back = Resources.Hint_ReverseSide)
                await ShowAnimationRotation(icon, back, isShow ? icon : null, data, isShow == false);
        }
    }
}
