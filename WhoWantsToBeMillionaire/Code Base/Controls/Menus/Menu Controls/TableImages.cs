...
namespace WhoWantsToBeMillionaire
{
    class TableImages : PictureBox, IDisposable
    {
        ...
        public TableImages(int rowsDistanse)
        {
            _images = new List<Image>();
            _bar = new GameVScrollBar();
            _distanseRows = rowsDistanse;
        ...
        {
            var height = _images.Sum(img => img.Height) + _distanseRows * (_images.Count - 1);
        ...
        public void AddText(string text, float fontSize, Size size, Color color, TextFormatFlags flags = TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter)
        {
            ...
            using (var g = Graphics.FromImage(image))
            using (var font = FontFactory.CreateFont(GameFonts.Arial, fontSize, FontStyle.Bold))
            {
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                _images.ForEach(image => image.Dispose());
                _images.Clear();
